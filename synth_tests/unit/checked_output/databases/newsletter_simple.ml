...
(*the state is defined by the data base*)
type nlrecord = 
			{nletter : string; 
		   	user : string ; 
		   	subscribed : bool ; 
...
type nldb =  ref [nlrecord];

(*a select operation on nldb with keys n and u *)
predicate nlmem : nldb :-> string :-> string :-> bool 
predicate ns : nldb :-> string :-> string :-> nlrecord  
