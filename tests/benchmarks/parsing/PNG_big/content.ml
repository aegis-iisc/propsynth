...
let len_content =
  ...
  let _init_ty_post =    P.Forall ( [(_h_ident, TyD.Ty_heap); (_bound_x, TyD.Ty_unit ) ;(_h'_ident, TyD.Ty_heap)],
 ...
 in 
  ...
  let pred_guard_2 =     P.Rel(RP.Grt ( MultiR ( (RelLang.instOfRel (RelId.fromString "sel")), [(_h_ident); (_cnt_ident) ]),
                           RelLang.V (Var(_bound_x))))in 




  let pred_guard = P.Conj ( pred_guard_0, P.Conj (pred_guard_1, pred_guard_2)) in 
 ...
 (*Creating step ty :: TODO ::we need a type_synthesis system for each step*)
   ...
   let l0 =  RP.Eq( MultiR ( (RelLang.instOfRel (RelId.fromString "sel")), [(_h_ident); (Var.fromString "content")]),           
                                          RelLang.relexpr_for_var var_l0 ) in 

   let var_l1 = Var.get_fresh_var "local" in 
   let l1 =  RP.Eq( MultiR ( (RelLang.instOfRel (RelId.fromString "sel")), [(_h'_ident); (Var.fromString "content")]),           
                                          RelLang.relexpr_for_var var_l1 ) in 

 
   let len_l0 =  RelLang.MultiR ( (RelLang.instOfRel (RelId.fromString "len")), [(Var.fromString var_l0)]) in 

                                        
   let len_l1 =  RelLang.MultiR ( (RelLang.instOfRel (RelId.fromString "len")), [(Var.fromString var_l1)]) in 


   let len_l0_plus_1 = RelLang.ADD (len_l0, RelLang.relexpr_for_int 1) in 
   ...
   let _d1_step_post =    P.Forall ( [(_h_ident, TyD.Ty_heap); (_bound_x, TyD.Ty_unit ) ;(_h'_ident, TyD.Ty_heap)],
                                         P.Conj(
                                          P.Rel(pred_cnt),
                                         (*_local_ = sel h' content /\ mem _local_ = 0*)
                                         len_sel 
  ...
  (*step body*)
   ...
   let inv_pred = 
     ...
     let len_sel =   RelLang.MultiR ( (RelLang.instOfRel (RelId.fromString "len")), [(var_ltemp)]) in 

     let sel_count =  RelLang.MultiR ( (RelLang.instOfRel (RelId.fromString "sel")), [(_h_ident);(Var.fromString "cnt")]) in 
 
     let len_sel_content_eq_count = RP.Eq (len_sel, sel_count) in 

     P.Conj (P.Rel(sel_ltemp), P.Rel(len_sel_content_eq_count)) in        
...
(* sel h cnt = bind_x*)
 ...
 (*Creating step ty :: TODO ::we need a type_synthesis system for each step*)
  ...
  (*step body*)
   let _d2_step_ = T_lam (T_var "x", (*place holder*) (T_var "x"), _d2_step_ty) in  
  ...
  * *)
   ...
   *x : \tau -> M {\h (inp -> _) v : int (\h, v, h'.  
                                                {v : p | snd v = s /\ fst v = len s}
   * *)
   (*TODO let annotated_post = P.True in*)
   let s_ident = Var.get_fresh_var "s" in 
   let annotated_post = P.Forall ( [(_h_ident, TyD.Ty_heap); (_x_ident, TyD.Ty_int) ;(_h'_ident, TyD.Ty_heap)],
                                      P.Conj (
                                                RP.NEq ( MultiR (RelLang.instOfRel (RelId.fromString "snd"), [(_v_ident)]), 
                                                        RelLang.relexpr_for_var (s_ident))
                                                ,   

                                                RP.NEq  ( MultiR (RelLang.instOfRel (RelId.fromString "fst"), [(_v_ident)]), 
                                                   MultiR (RelLang.instOfRel (RelId.fromString "len"), [(s_ident)]))
