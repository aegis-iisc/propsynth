...
Node : {name : [char]| true} -> {ind : int | true} -> {sub : [tree] | offsidetreelist sub} -> { v: tree | leaf (v) = false /\ indent (v) = ind /\ children (v) = sub};


step1 :  State {\(h:heap). true} unit {\(h:heap), (v:unit), (h' : heap). sel (h', out) == i};
step2 : State  {\(h:heap). true} unit {\(h:heap), (v:unit), (h' : heap). h = h'};

finalqf : State {\(h:heap).true} tree {\(h:heap), (v:tree), (h' : heap). sel (h', ind) = sel (h, ind) /\ sel(h, ind) = I /\ leaf (v) = true /\ ident (v) > I};
finalother : State  {\(h:heap).true} err {\(h:heap), (v:unit), (h' : heap). h = h'};
