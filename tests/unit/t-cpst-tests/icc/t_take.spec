...
formula guardloop = \(h : heap).  plssel (h, inpp) = l0  /\ phd (l0) = {(ip)} /\ (bn > (sel (h, cnt)));
formula guardexit = \(h : heap).  plssel (h, inpp) = le  /\ phd (le) = {(ip)} /\ sel (h, cnt) == bn;

formula inv = \(h : heap). plssel (h, out) = o /\ (plen (o) == sel (h, cnt)) ;
...
finalqf : State {\(h:heap). true} 
		unit 
		{\(h : heap), (v: unit), (h' : heap). plssel (h', inpp) = plssel (h, out)};
