...
let specfile = "tests/unit/t-cpst-tests/icc/t_tag_choice1.spec"  
...
(*define transducer*)     
let t =                    
  let states_st = ["q0";"qf1";"qf2"] in 

       (* let regs = [(res, SpecLang.RefTy.fromTyD (SpecLang.TyD.Ty_int))] in *)
       let regs = [(res, getType "res")] in 

        let d1 = Trans.Del {name = "d1";src = "q0"; tgt = "qf1"; guard=getFormula ("guard1"); step=step1; inv = SpecLang.P.True} in 

        let d2 = Trans.Del {name = "d1";src = "q0"; tgt = "qf2"; guard=getFormula ("guard2"); step=step1; inv = SpecLang.P.True} in 

        let final = Trans.Final {fin = fun (st:Trans.state) -> 
                                match st with 
                                   | "qf1" -> (finalqf, getType ("finalqf1"))
                                   | "qf2" -> (finalqf, getType ("finalqf2"))
        ...
        Trans.Lambda (T_var "sig", getType "sig", Trans.ST transition_system)

let () = 
   Morpheus.verifyCPST initGamma t (getType "annot") false
