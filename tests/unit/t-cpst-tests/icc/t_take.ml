...
open SB  
open Trans
let specfile = "tests/unit/t-cpst-tests/icc/t_take.spec"  
...
let steploop =  T_lam ("x", (T_var "x"), getType "steploop") 
let stepexit = T_lam ("x", (T_var "x"), getType "stepexit") 
...
let t = 
        ...
        let _d1 = Trans.Del {name = "_d1";src = "q0"; tgt = "q0"; guard=getFormula "guardloop"; step=(steploop); inv = getFormula ("inv")} in
        let _d2 = Del {name = "_d2";src = "q0"; tgt = "qf"; guard=getFormula "guardexit"; step=(stepexit); inv = True} in 
        ...
        let pst = 
           Trans { regs = regs;
            ...
            deltas = [(_d1, getType ("steploop"));(_d2, getType "stepexit")];
            annot = getType "annot"}

          
        in 
        Trans.Lambda (T_var "bn", getType "bn", ST pst)
