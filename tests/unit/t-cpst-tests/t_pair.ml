...
let specfile = "tests/unit/t-cpst-tests/t_pair.spec"
...
let t_pair =                    
       ...
       (* let regs = [(res, SpecLang.RefTy.fromTyD (SpecLang.TyD.Ty_int))] in *)
        ...
        let transition_system =  Trans.Trans { regs = regs;
                            ...
                            annot = getType ("annot")}
        in    
                        (*\one. return Pair (zero, one)*)
                        Trans.Lambda (T_var "one", getType "one",  Trans.ST transition_system)

let () = 
   Morpheus.verifyCPST gamma t_pair (getType "annot")     
