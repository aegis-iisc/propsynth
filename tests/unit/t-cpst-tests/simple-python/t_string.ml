...
(*the spec for string s is too complex, 
 * try a simpler s*)
...
open SB  
open Trans
let specfile = "tests/unit/t-cpst-tests/simple-haskell/t_string.spec"  
...
let t_string = 
        ...
        let regs = [(T_var "out",getType "out"); 
                    (T_var "pat", getType "pat"); (T_var "start", getType "start")] in 
        ...
        in 
        Trans.Lambda (T_var "str", getType "str", ST pst)

let () = 
   Morpheus.verifyCPST gamma t_string (getType "annot") false
