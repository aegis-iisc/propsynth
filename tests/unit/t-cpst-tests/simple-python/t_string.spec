...
nil : int; 
emptytreelist : [tree];
formula guardloop = \(h : heap).   pssel (h, iinp) = l0  /\ hd (l0) == i /\ 
...
formula guardexit =  \(h : heap).    ilssel (h, pat) = l4 /\ hd (l4) == nil;   
...
finalqf : State {\(h:heap). true} tree {\(h : heap), (v: tree), (h' : heap). node (v) = ilssel (h', out) /\ indent (v) == start /\ children (v) = emtyptreelist};
finalother : State  {\(h : heap). true} unit {\(h : heap), (v : unit), (h' : heap). h = h'};

annot : State  {\(h:heap). true} tree {\(h : heap), (v: tree), (h' : heap).  node (v) = ilssel (h', out) /\ indent (v) == start /\ children (v) = emptytreelist};
