...
let specfile = "tests/unit/t-cpst-tests/xauction/t_seller_info_action.spec"  
       ...
       (* let regs = [(res, SpecLang.RefTy.fromTyD (SpecLang.TyD.Ty_int))] in *)
        ...
        in    
                        ...
                        (*Trans.Lambda (T_var "sname", getType "sname",
                             Trans.Lambda( T_var "rating", getType "rating", *)
                                Trans.ST transition_system


let () = 
   Morpheus.verifyCPST gamma seller_info_action (getType "annot") true     
