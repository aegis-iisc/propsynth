...
let specfile = "tests/unit/t-cpst-tests/xauction/t_bidder_info.spec"  
 ...
 * t_bidder_name = 
    ...
    *)
let t_bidder_info = 
        let t_open =  Trans.Red (T_const (SLit "<bidderinfo>"), T_string.t_sring) in 
  let t_close =  Trans.Red (T_const (SLit "<\bidderinfo>"), T_string.t_sring) in 

        let t_bidder_name = T_bidder_name.t_bidder_name in 
        let t_bidder_rating = T_bidder_rating.t_seller_rating in 
        let t_action = T_action.bidder_action in  
...
let () = 
   Morpheus.verifyCPST gamma t_bidder_info (getType "t_bidder_info") false  
