...
let len_trans = 
  ...
  let _d1_decr_ty = RefTy.MArrow (Effect.Pure, decr_pre,  Ty_int, decr_post) in 
 ...
 (*Creating step ty*)
  ...
  let _d1_step_ty = RefTy.MArrow (Effect.Pure,_d1_step_pre, Ty_unit,  _d1_step_post) in 
 ...
 in 
  let _init_ty_post =    P.Forall ( [(_h_ident, TyD.Ty_heap); (_x_ident, TyD.Ty_int) ;(_h'_ident, TyD.Ty_heap)],
            ...
            in 
   let _init_term = T_seq  ( (T_assign( (T_var "cnt"), (T_cref (T_const (ILit 3))))),
                       (T_assign ( (T_var "inp"), (T_cref (T_const (Llit [])))))) in 
 (* let _init_ty_post = Op( Uupdate (Uterm (T_var "h'"), Uterm (T_var "cnt"), (Uterm (T_const (ILit 0))))) in *)
   let _init_ty = RefTy.MArrow (Effect.Pure,_init_ty_pre, Ty_unit, _init_ty_post ) in 
